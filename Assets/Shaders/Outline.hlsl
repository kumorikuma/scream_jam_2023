...
void Outline_float(float2 UV, float OutlineThickness, float DepthSensitivity, float NormalsSensitivity, float ColorSensitivity, float4 OutlineColor, out float4 OutColor)
{
    ...
    // OutColor = ((1 - edge) * original) + (edge * lerp(original, OutlineColor,  OutlineColor.a));
    
    // OutColor = SAMPLE_TEXTURE2D(_CameraNormalsTexture, sampler_CameraNormalsTexture, uvSamples[0]);
    // 

    // Normals Debugging
    float2 normals = SAMPLE_TEXTURE2D(_CameraDepthNormalsTexture, sampler_CameraDepthNormalsTexture, uvSamples[0]).rg;
    float4 normalsColor = float4(normals.x, normals.y, 0, 1);
    // OutColor = ((1 - edge) * normalsColor) + (edge * OutlineColor);

    // Depth Debugging
    // OutColor = SAMPLE_TEXTURE2D(_CameraDepthTexture, sampler_CameraDepthTexture, uvSamples[0]);
